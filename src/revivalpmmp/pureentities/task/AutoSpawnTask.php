...
class AutoSpawnTask extends Task{
	...
	const NETHER_HOSTILE_CAP_CONSTANT = 70;
		...
		foreach($this->plugin->getServer()->getLevels() as $level){
			...
			if(count($level->getPlayers()) > 0){
				...
				if(($totalChunks = count($spawnMap)) > 0){
					...
					$netherHostileCap = self::NETHER_HOSTILE_CAP_CONSTANT * $totalChunks / 256;
	...
	protected function spawnPackToLevel(Vector3 $center, int $entityId, Level $level, string $type, bool $isBaby = false){
		...
		for($attempts = 0; $attempts <= 16 and $currentPackSize < $maxPackSize; $attempts++){
			...
			//PassiveWet Spawn
			if($type == "passiveWet" and $this->isValidWetSpawnLocation($pos) and $this->isSpawnAllowedByBiome($entityId, $level->getBiomeId($x, $z))){
				PureEntities::logOutput("AutoSpawnTask: Spawning Mob (ID = $entityId) to location: $x, $center->y, $z");
				$success = PureEntities::getInstance()->scheduleCreatureSpawn($pos, $entityId, $level, $type, $isBaby) !== null;
				if($success)
				{
			...
			//Nether Hostile Spawn
			if($type == "netherHostile" and $this->isValidDrySpawnLocation($pos) and $this->isSpawnAllowedByBiome($entityId, $level->getBiomeId($x, $z))){
				PureEntities::logOutput("AutoSpawnTask: Spawning Mob (ID = $entityId) to location: $x, $center->y, $z");
				$success = PureEntities::getInstance()->scheduleCreatureSpawn($pos, $entityId, $level, $type, $isBaby) !== null;
				if($success)
				{
	...
	private function isValidWetSpawnLocation(Position $spawnLocation){
		if(!$spawnLocation->level->getBlockAt($spawnLocation->x, $spawnLocation->y - 1, $spawnLocation->z)->isTransparent()
			and ($spawnLocation->level->getBlockAt($spawnLocation->x, $spawnLocation->y, $spawnLocation->z)->isTransparent() and
				$spawnLocation->level->getBlockAt($spawnLocation->x, $spawnLocation->y, $spawnLocation->z)->getId() == Block::WATER)
			and ($spawnLocation->level->getBlockAt($spawnLocation->x, $spawnLocation->y + 1, $spawnLocation->z)->isTransparent()
	...
	private function spawnPassiveWetMob(Chunk $chunk, Level $level){
		PureEntities::logOutput("AutoSpawnTask: Attempting to spawn passive wet mob.");
		$packCenter = $this->getRandomLocationInChunk(new Vector2($chunk->getX(), $chunk->getZ()));
		$lightLevel = $level->getFullLightAt($packCenter->x, $packCenter->y, $packCenter->z);
		if($this->isValidPackCenter($packCenter, $level)) 
		{
			$mobId = Data::NETWORK_IDS[MobTypeMaps::PASSIVE_WET_MOBS[array_rand(MobTypeMaps::PASSIVE_WET_MOBS)]];
			$this->spawnPackToLevel($packCenter, $mobId, $level, "passiveWet");
		}
		PureEntities::logOutput("AutoSpawnTask: Not a valid pack center.");
	...
	private function spawnNetherHostileMob(Chunk $chunk, Level $level){
		PureEntities::logOutput("AutoSpawnTask: Attempting to spawn Nether hostile mob.");
		$packCenter = $this->getRandomLocationInChunk(new Vector2($chunk->getX(), $chunk->getZ()));
		PureEntities::logOutput("AutoSpawnTask: Chosen Pack Center at $packCenter->x, $packCenter->y, $packCenter->z.");
		$lightLevel = $level->getFullLightAt($packCenter->x, $packCenter->y, $packCenter->z);
		PureEntities::logOutput("AutoSpawnTask: light level at trial pack center is $lightLevel");
		if($this->isValidPackCenter($packCenter, $level)){
			...
			PureEntities::logOutput("AutoSpawnTask: light level at valid pack center is $lightLevel");
			//Spawn Nether Hostile Mobs
			$mobId = Data::NETWORK_IDS[MobTypeMaps::NETHER_HOSTILE_MOBS[array_rand(MobTypeMaps::NETHER_HOSTILE_MOBS)]];
			$this->spawnPackToLevel($packCenter, $mobId, $level, "hostileNether");
		}
		PureEntities::logOutput("AutoSpawnTask: Not a valid pack center.");
	}
